...
一个用于 **Unreal Engine 5.0 – 5.6+** 的编辑器插件，通过自然语言描述自动生成蓝图节点，并提供 AI 驱动的蓝图分析与自我修复能力。
...
- **蓝图分析报告**：一键分析当前打开的蓝图，AI 以中英双语形式输出整体总结和分模块详细说明，结果展示在独立弹窗中
...
1. 在 UE 编辑器中**打开一个蓝图**
2. 点击面板中的 **Analyze Blueprint** 按钮
3. 状态日志显示 `[Analyze] Analysis complete.` 后，弹出分析报告窗口
4. 报告包含中英双语的整体总结，以及按逻辑用途分组的模块说明

**报告示例：**

```
## 总结
这个蓝图负责玩家角色的战斗逻辑，包含生命值管理、受伤响应和死亡处理。

## Summary
This Blueprint handles player combat, including health management,
damage response, and death handling.

## 模块分析

### 初始化
说明：BeginPlay 触发时将 Health 设为 100 并启用碰撞检测。
Description: On BeginPlay, sets Health to 100 and enables collision.
涉及节点：Event BeginPlay, Set Health, Set Collision Enabled

### 受伤处理
说明：自定义事件 TakeDamage 减少 Health，触发受击动画。
Description: Custom event TakeDamage decrements Health and plays hit animation.
涉及节点：Custom Event TakeDamage, Set Health, Play Animation
```

> 分析功能使用独立的系统提示词，与节点生成互不干扰；`Custom System Prompt` 设置对分析请求无影响。
...
SAIBPAssistantWidget          ← UI 层：Slate 面板，渲染控件，响应用户操作
    ...
    │
    ├──► SAIBPAnalysisDialog   ← 分析层：模态窗口，展示双语分析报告
    │
...
2. UAIBPContextUtils::GetActiveBlueprintData()
       │  返回 { baseClass, variables[], components[], functions[], interfaces[] }
       │
3. UAIBPContextUtils::GetActiveBlueprintGraphData()
       │  返回 { blueprintName, graphs[{ name, nodes[{ title, class }] }] }
       │
4. UAIBPHttpService::SendAnalysisRequest()
       │  使用独立分析提示词（不读取 CustomSystemPrompt）
       │  POST → AI API（后台线程）
       │
5. AsyncTask(GameThread) → SAIBPAnalysisDialog 模态弹窗展示双语报告
```
...
`GetActiveBlueprintGraphData()` 遍历蓝图所有事件图和自定义函数图，将每个节点的标题（`GetNodeTitle`）和类名序列化为 JSON。与变量/组件等上下文数据一同发送给 AI，并使用独立的分析系统提示词（要求输出固定格式的双语报告）。分析功能不修改蓝图，也不写入对话历史。
...
└── Source/
    └── AIBlueprintAssistant/
        ...
        │   ├── SAIBPAnalysisDialog.h      # 蓝图分析报告模态对话框
        │   ├── AIBPContextUtils.h         # 蓝图上下文 + 图表节点提取工具
        │   ├── AIBPNodeFactory.h          # T3D 节点导入工厂
        │   └── AIBPHttpService.h          # 异步 HTTP 服务（生成 + 分析）
        └── Private/
            ├── AIBlueprintAssistant.cpp
            ├── AIBPSettings.cpp
            ├── SAIBPAssistantWidget.cpp
            ├── SAIBPPreviewDialog.cpp
            ├── SAIBPAnalysisDialog.cpp
...
- 确认当前有蓝图编辑器处于打开状态（Analyze Blueprint 按钮会在无蓝图时输出 `[Warning]`）
- 若报告格式混乱，可能是模型未遵循输出格式；尝试使用更强的模型（如 gpt-4o）或降低 Temperature
- 分析功能使用独立提示词，`Custom System Prompt` 设置不影响分析结果
